...
祝前程似锦，未来可期。🌙`,
  ...
  const AI_REAL = (() => {
    ...
    // 返回 { real: {rank, category, source} | null, estimate, tier }
    async function realRank (score, province, year = 2024) {
      try {
        const url = `${baseURL()}/api/realdata/rank?score=${score}&province=${encodeURIComponent(province)}&year=${year}`;
