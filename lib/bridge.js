  ...
  function showRealScoreForm () {
    ...
    overlay.innerHTML = `
      <div class="gk-realscore__box">
        <h2>📊 现实路线 · 输入你的真实分数</h2>
        <p>省份：<b>${prov}</b> · 选科：<b>${group}</b> · 满分：<b>${total}</b></p>
    ...
    overlay.querySelector('#realSubmit').addEventListener('click', async () => {
      ...
      overlay.querySelector('#realResult').innerHTML = '<span style="color:#9ab">⏳ 正在估算…</span>';
      try {
        const resp = await fetch('http://localhost:8001/api/analyze', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ province: prov, group, score, total, subjects: [] }),
        });
        const data = await resp.json();
        let html = `<div class="gk-realscore__rank">📊 估算位次：约 <b>#${(data.localRank||0).toLocaleString()}</b></div>`;
        if (data.ai && data.ai.advice) html += `<div class="gk-realscore__advice">💡 ${data.ai.advice}</div>`;
        if (data.hasAI === false) html += `<div class="gk-realscore__note">⚠️ AI 服务未配置（本地估算模式）。启动 server-ai.mjs 并配置 .env 获得精确建议。</div>`;
        overlay.querySelector('#realResult').innerHTML = html;
        overlay.querySelector('#realScore2').value = score;
        overlay.querySelector('#realUse').style.display = 'block';
        overlay.querySelector('#realUse').dataset.score = score;
      } catch (e) {
        overlay.querySelector('#realResult').innerHTML = '<span style="color:#e53170">⚠️ AI 服务未启动。请运行 node server-ai.mjs</span>';
      }
    });
    overlay.querySelector('#realUse').addEventListener('click', () => {
      const score = parseInt(overlay.querySelector('#realUse').dataset.score, 10);
      const g2 = get();
      // 用真实分数覆盖虚拟分数
      const tier = score / total >= 0.87 ? 'S' : score / total >= 0.8 ? 'A' : score / total >= 0.73 ? 'B' : score / total >= 0.67 ? 'C' : 'D';
      set({ score, total, tier, rank: 0, autoMode: false });
      overlay.remove();
