...
const GK = (() => {
  ...
  function showRealScoreForm () {
    ...
    overlay.querySelector('#realSubmit').addEventListener('click', async () => {
      ...
      // 第二步：异步拉取真实一分一段表数据（1-2 秒），覆盖显示
      window.AGENT.AI_REAL.realRank(score, provName, 2024).then(realData => {
        if (realData?.real?.rank) {
          quickRank = realData.real.rank;
          rankSource = '✓ 真实数据(' + realData.real.category + ')';
          const display = document.getElementById('rankDisplay');
          if (display) {
            display.innerHTML = `📊 全省位次：<b>#${quickRank.toLocaleString()}</b> <span style="color:#7c9">（${realData.real.category} · ${rankSource}）</span>`;
          }
