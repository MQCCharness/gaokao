...
const GK = (() => {
  ...
  // 语音开关（存 localStorage，默认开启；voice() 开头检查）
  function voiceEnabled () {
    try { return localStorage.getItem('gk_voice_on') !== '0'; } catch (e) { return true; }
  }
  function setVoiceEnabled (on) {
    try {
      localStorage.setItem('gk_voice_on', on ? '1' : '0');
      // 同步更新浮动按钮 UI（class + 图标 + title）
      const btn = document.querySelector('.gk-voice-toggle');
      if (btn) {
        btn.classList.toggle('gk-voice-toggle--off', !on);
        btn.innerHTML = on ? '🔊' : '🔇';
        btn.title = on ? '语音：开（点击关闭）' : '语音：关（点击开启）';
      }
      // 关闭时立刻停掉当前播放
      if (!on && _voiceAudio) { _voiceAudio.pause(); _unlockTextBox(); }
    } catch (e) {}
  }
  ...
  function showInterestPicker () {
    ...
    overlay.querySelector('.gk-interestpick__confirm').addEventListener('click', () => {
      ...
      sfx('select');
      overlay.remove();
      setTimeout(() => { try { monogatari.run('jump InterestTaskDone'); } catch (e) {} }, 50);
    });
  }
  ...
  const MAP_NODES = [
    ...
    { key: 'corridor', label: '走廊 · 同学们', icon: '🚶', dest: 'NpcCorridorEnter', bg: 'scene-corridor', bgm: 'bgm-chat', npc: '林/小雨/大志', shard: '', optional: true,
      pos: { x: 26.3, y: 43 }, requires: ['score'], lockReason: '🔒 先去教室查分——有了分数，才能和同学们对比处境。' },
    // 放松场景（日常支线，节奏调节 + 关系培养；optional，无需前置）
    { key: 'relax_gym',   label: '体育馆 · 打篮球', icon: '🏀', dest: 'RelaxGym', bg: 'scene-gym', bgm: 'bgm-chat', npc: '阿星', shard: '', optional: true,
      pos: { x: 75, y: 20 }, requires: [], lockReason: '' },
    { key: 'relax_star',  label: '屋上 · 看星空',   icon: '🌌', dest: 'RelaxStargaze', bg: 'scene-stargaze', bgm: 'bgm-vision', npc: '学姐·温', shard: '', optional: true,
      pos: { x: 16, y: 8 }, requires: [], lockReason: '' },
    { key: 'relax_river', label: '河川敷 · 散步',   icon: '🌊', dest: 'RelaxRiver', bg: 'scene-river', bgm: 'bgm-chat', npc: '死党·阿星', shard: '', optional: true,
      pos: { x: 87.5, y: 60 }, requires: [], lockReason: '' },
    // 家（家人互动，查分后解锁）
    { key: 'home', label: '家 · 家人', icon: '🏠', dest: 'HomeEnter', bg: 'scene-home', bgm: 'bgm-chat', npc: '妈妈/爸爸/小姨', shard: '', optional: true,
      pos: { x: 3.5, y: 78 }, requires: ['score'], lockReason: '🔒 先去教室查分——家人会问你考了多少。' },
    // 教师办公室（老师互动，查分后解锁）
    { key: 'office', label: '办公室 · 老师', icon: '📋', dest: 'OfficeEnter', bg: 'scene-office', bgm: 'bgm-chat', npc: '李老师/王主任', shard: '', optional: true,
      pos: { x: 4, y: 32 }, requires: ['score'], lockReason: '🔒 先去教室查分——老师要看了分数才能给建议。' },
  ];
  ...
  function showSaveWarn (msg) {
    ...
    banner.innerHTML = `
      <div class="gk-savewarn__icon">⚠️</div>
      <div class="gk-savewarn__body">
        <div class="gk-savewarn__text">${msg || '即将进入关键节点，建议先存档，否则可能错过某些分支。'}</div>
        <div class="gk-savewarn__howto">
          <span class="gk-savewarn__key">💾 存档方法：</span>
          <b>点下方「立即存档」</b>，或点游戏右上角 <b>「存档」</b> 按钮
        </div>
      </div>
      <div class="gk-savewarn__btns">
        <button class="gk-savewarn__btn gk-savewarn__btn--save">💾 立即存档</button>
        <button class="gk-savewarn__btn gk-savewarn__btn--ok">知道了</button>
      </div>`;
    document.body.appendChild(banner);
    const dismiss = () => banner.remove();
    // 知道了：关闭横幅
    banner.querySelector('.gk-savewarn__btn--ok').addEventListener('click', dismiss);
    // 立即存档：直接打开引擎的存档界面
    banner.querySelector('.gk-savewarn__btn--save').addEventListener('click', () => {
      try { monogatari.run('open save-screen'); } catch (e) {
  ...
  // ══════ 结局照片蒙太奇（Photo Montage）══════
  // 在桌面上一张张浮现旧照片，把整局情绪推到顶
  // mood: 'good' / 'normal' / 'bitter' 影响色调与节奏
  function showPhotoMontage (mood = 'good') {
    const old = document.querySelector('.gk-montage');
    if (old) old.remove();

    // 6 张照片（由 mimo-v2.5 设计，sharp 合成）
    const photos = [
      { id: 1, caption: '春节·全家福', narration: '妈妈笑着搂住你的肩。' },
      { id: 2, caption: '篮球场·阿星', narration: '阿星说赢了球要请你喝汽水。' },
      { id: 3, caption: '办公室·李老师', narration: '李老师说，你的未来不止于此。' },
      { id: 4, caption: '教室·凛学姐', narration: '凛学姐说，你其实很努力。' },
      { id: 5, caption: '屋顶·温学姐', narration: '温学姐说，每颗星都是一个梦想。' },
      { id: 6, caption: '毕业典礼', narration: '毕业照上，每个人都笑得很灿烂。' },
    ];

    // 不同结局的氛围（影响最终句）
    const finalLine = {
      good:   '笔尖落下，是你对这些瞬间的回答，也是未来的起点。',
      normal: '笔尖落下，那些瞬间仿佛在说：再来一次，可以更好。',
      bitter: '笔尖悬着——那些瞬间，你没资格回答。',
    }[mood] || '笔尖落下，是你对这些瞬间的回答。';

    const overlay = document.createElement('div');
    overlay.className = 'gk-montage gk-montage--' + mood;

    // 桌面背景 + 顶部文字 + 照片区 + 旁白 + 最终句 + 关闭按钮
    overlay.innerHTML = `
      <div class="gk-montage__bg"></div>
      <div class="gk-montage__inner">
        <div class="gk-montage__hint">笔尖悬停的瞬间——</div>
        <div class="gk-montage__desk">
          ${photos.map((p, i) => `
            <div class="gk-montage__photo" data-idx="${i}" style="transform: rotate(${(i - 2.5) * 4}deg) translate(${(i - 2.5) * 8}px, ${Math.abs(i - 2.5) * 6}px);">
              <img src="assets/montage/photo_${p.id}.png" alt="${p.caption}"/>
            </div>
          `).join('')}
        </div>
        <div class="gk-montage__caption" data-cur="-1">
          <span class="gk-montage__cap-text"></span>
        </div>
        <div class="gk-montage__final" hidden>${finalLine}</div>
        <button class="gk-montage__next">▶ 点击桌面</button>
      </div>`;

    document.body.appendChild(overlay);

    const photoEls = overlay.querySelectorAll('.gk-montage__photo');
    const captionEl = overlay.querySelector('.gk-montage__cap-text');
    const finalEl = overlay.querySelector('.gk-montage__final');
    const nextBtn = overlay.querySelector('.gk-montage__next');
    let cur = -1;
    ...
    function step () {
      ...
      if (cur < photos.length) {
        // 浮现当前照片
        photoEls[cur].classList.add('gk-montage__photo--show');
        // 切换旁白（淡出再淡入）
        captionEl.style.opacity = '0';
        setTimeout(() => {
          captionEl.textContent = photos[cur].narration;
          captionEl.style.opacity = '1';
        }, 250);
        // 最后一张时改变按钮文案
        if (cur === photos.length - 1) {
          nextBtn.textContent = '▶ 看完整桌面';
        }
      } else {
        // 全部已浮现 → 显示最终句
        finalEl.hidden = false;
        finalEl.classList.add('gk-montage__final--show');
        nextBtn.textContent = '🌙 收下这些瞬间';
        nextBtn.onclick = () => {
  ...
  // ══════ 众声喧哗 Visions（人生理想关后，8 位科技大佬化名短剧情）══════
  // chapter: 1-4，每章弹出 1-3 张 vision 卡片 + 旁白
  // 卡片从屏幕外滑入、悬浮、可点击关闭后推进剧情
  // 第 3 章多卡同时出场（信息过载感）
  function showVisions (chapter, onClose) {
    const old = document.querySelector('.gk-visions');
    if (old) old.remove();

    // 4 章内容（来自 mimo-v2.5 设计，台词已改写合规）
    const CHAPTERS = {
      1: {
        narration: '你刚放下笔，手机推送了一条新闻，标题闪着金光。',
        visions: [
          { id: 'huang', name: '皮衣教父', title: 'GPU 教父',
            line: '"算力是新石油，孩子。你未来的竞争力，从选择哪块『油田』开始。"' },
        ],
      },
      2: {
        narration: '你还没看完，更多的推送像潮水般涌来，淹没你的视野。',
        visions: [
          { id: 'musk', name: '火箭狂人', title: '钢铁侠',
            line: '"四年，AI 就能接管大部分脑力活。你要么造它，要么…被它淘汰。"' },
          { id: 'altman', name: 'GPT 之父', title: 'OpenAI 掌门',
            line: '"我们正站在通用智能的门槛上。你毕业那天，世界会完全不同。"' },
        ],
      },
      3: {
        narration: '屏幕彻底被占领。每个窗口都在对你呐喊，每个声音都代表一个『未来』。',
        visions: [
          { id: 'feifei', name: 'AI 教母', title: 'ImageNet 之母',
            line: '"机器需要『看懂』世界。空间智能，才是下一场认知革命的核心。"' },
          { id: 'leijun', name: '闪电 CEO', title: '造车新势力',
            line: '"永远相信美好的事即将发生。ALL IN，赌上一切，才能换来一线生机。"' },
          { id: 'zhouhy', name: '红衣大叔', title: '安全老兵',
            line: '"别怕 AI 抢饭碗。它淘汰的，是那些拒绝使用它的人。"' },
        ],
      },
      4: {
        narration: '你感到窒息。他们的宏图如此耀眼，可为什么你的手在发抖？',
        visions: [
          { id: 'luoyh', name: '老罗', title: '理想主义创业者',
            line: '"彪悍的人生不需要解释。现在不折腾，这辈子就稳了——稳稳地平庸。"' },
        ],
      },
    };
    ...
    const overlay = document.createElement('div');
    overlay.className = 'gk-visions gk-visions--ch' + chapter;

    // 卡片随机角度+位置（让画面有"信息过载"感）
    const positions = chapter === 3
      ? [{ x: 0, y: 0, r: -6 }, { x: 0, y: 0, r: 4 }, { x: 0, y: 0, r: -2 }]  // 第3章叠加
      : ch.visions.map(() => ({ x: 0, y: 0, r: (Math.random() - 0.5) * 8 }));

    overlay.innerHTML = `
      <div class="gk-visions__stream"></div>
      <div class="gk-visions__inner">
        <div class="gk-visions__narration">${ch.narration}</div>
        <div class="gk-visions__cards">
          ${ch.visions.map((v, i) => `
            <div class="gk-visions__card" style="transform: rotate(${positions[i].r}deg); animation-delay: ${i * 0.4}s;" data-idx="${i}">
              <img src="assets/visions/vision_${v.id}.png" alt="${v.name}"/>
              <div class="gk-visions__line">${v.line}</div>
              <div class="gk-visions__hint">▸ 点击继续</div>
            </div>
          `).join('')}
        </div>
      </div>`;
    ...
    // 点击任意卡片关闭整个 overlay（剧情继续）
    overlay.querySelectorAll('.gk-visions__card').forEach(card => {
      card.addEventListener('click', () => {
        overlay.classList.add('gk-visions--closing');
        setTimeout(() => { overlay.remove(); if (onClose) onClose(); }, 700);
      });
    });

    // 也允许点背景关闭
    overlay.addEventListener('click', (e) => {
      if (e.target === overlay || e.target.classList.contains('gk-visions__stream')) {
        overlay.classList.add('gk-visions--closing');
        setTimeout(() => { overlay.remove(); if (onClose) onClose(); }, 700);
      }
    });
  ...
  function showNpcInteract (classmateId, optionKey, returnLabel) {
    // 查所有 NPC（ALL_NPCS 含同学+家人+老师；旧版只查 CLASSMATES 导致家人/老师弹层失败）
    const list = window.ALL_NPCS || window.CLASSMATES || [];
    const c = list.find(x => x.id === classmateId);
    if (!c) return;
    // close 后跳转目标：优先用传入的 returnLabel，否则按 NPC 类型自动判断
    const dest = returnLabel || (classmateId.startsWith('fam_') ? 'HomeAfter'
      : classmateId.startsWith('tch_') ? 'OfficeAfter'
      : 'NpcCorridorAfter');
  ...
  // —— NPC 互动对象选择弹层（替代引擎 Choice，支持"已互动"标记）——
  // options: { title, subtitle, npcs: [{id, label, icon, talkedKey, jumpLabel}], leaveLabel, leaveJump, returnCheck }
  // 已互动的 NPC（_talked[talkedKey] 为 true）显示 ✓ 标记 + 变灰，点击弹"已互动"提示
  function showNpcPicker (opts) {
    const old = document.querySelector('.gk-npcpicker');
    if (old) old.remove();
    const g = GK.get();
    const talked = g._talked || {};
    const allTalked = opts.npcs.every(n => talked[n.talkedKey]);
    const cards = opts.npcs.map(n => {
      const isTalked = !!talked[n.talkedKey];
      return `<div class="gk-npcpicker__card${isTalked ? ' gk-npcpicker__card--done' : ''}" data-jump="${n.jumpLabel}" data-talked="${isTalked}">
        <div class="gk-npcpicker__icon">${n.icon}</div>
        <div class="gk-npcpicker__label">${n.label}</div>
        ${isTalked ? '<div class="gk-npcpicker__badge">✓ 已互动</div>' : '<div class="gk-npcpicker__badge gk-npcpicker__badge--go">▶ 点击互动</div>'}
      </div>`;
    }).join('');
    const overlay = document.createElement('div');
    overlay.className = 'gk-npcpicker';
    overlay.innerHTML = `
      <div class="gk-npcpicker__box">
        <h2 class="gk-npcpicker__title">${opts.title}</h2>
        <p class="gk-npcpicker__subtitle">${opts.subtitle}${allTalked ? '<br><b style="color:#7f5af0">已和所有人互动过，可选择离开</b>' : ''}</p>
        <div class="gk-npcpicker__grid">${cards}</div>
        <div class="gk-npcpicker__footer">
          <button class="gk-npcpicker__leave">${opts.leaveLabel || '🚪 离开'}</button>
        </div>
      </div>`;
    document.body.appendChild(overlay);
    // 点 NPC 卡片
    overlay.querySelectorAll('.gk-npcpicker__card').forEach(card => {
      card.addEventListener('click', () => {
        const isTalked = card.getAttribute('data-talked') === 'true';
        if (isTalked) {
          // 已互动 → 弹"已互动"提示（抖动卡片）
          card.style.animation = 'none';
          void card.offsetWidth; // 触发重排
          card.style.animation = 'gk-shake 0.4s';
          const badge = card.querySelector('.gk-npcpicker__badge');
          if (badge) {
            const oldText = badge.textContent;
            badge.textContent = '⚠ 已互动过，请选其他人';
            badge.style.color = '#e53170';
            setTimeout(() => { badge.textContent = oldText; badge.style.color = ''; }, 1500);
          }
          sfx('click');
          return;
        }
        ...
        sfx('select');
        overlay.remove();
        // 用 setTimeout 延迟 jump，等引擎完成当前标签语句执行（直接 jump 会被后续语句覆盖）
        setTimeout(() => { try { monogatari.run('jump ' + jump); } catch (e) {} }, 300);
      });
    });
    // 离开按钮
    overlay.querySelector('.gk-npcpicker__leave').addEventListener('click', () => {
      sfx('click');
      overlay.remove();
      const dest = opts.leaveJump || 'CampusMap';
      setTimeout(() => { try { monogatari.run('jump ' + dest); } catch (e) {} }, 50);
    });
  }
  ...
  function showCampusMap () {
    ...
    if (old) old.remove();
    // 停掉所有环境音（避免上一个场景的 env-* 循环音带进地图/下一个场景）
    try { monogatari.run('stop sound'); } catch (e) {}
    const g = get();
    ...
    // 真校园地图：SVG 背景 + 节点 marker 钉在建筑物上
    const markers = MAP_NODES.map((n) => {
      const isDone = !!cleared[n.key];
      const unlocked = isUnlocked(n.key);
      const state = isDone ? 'done' : (unlocked ? 'open' : 'locked');
      const clickable = unlocked && !isDone;
      const tip = isDone ? '✓ 已完成' : (unlocked ? '▶ 前往' : (n.lockReason || '🔒 未解锁'));
      const cls = 'gk-map__marker gk-map__marker--' + state + (n.optional ? ' gk-map__marker--opt' : '');
      const dataAttrs = clickable ? `data-dest="${n.dest}" data-task="${n.key}"` : (isDone ? '' : `data-tip="${escapeHtml(n.lockReason || '未解锁')}"`);
      return `<button class="${cls}" style="left:${n.pos.x}%; top:${n.pos.y}%;" ${dataAttrs} title="${escapeHtml(n.label)}">
        <span class="gk-map__marker-emoji">${isDone ? '✅' : (unlocked ? n.icon : '🔒')}</span>
        <span class="gk-map__marker-label">${n.label.split('·')[0].trim()}</span>
        <span class="gk-map__marker-tip">${tip}</span>
      </button>`;
    }).join('');

    const overlay = document.createElement('div');
    overlay.className = 'gk-map gk-map--graphic';
    overlay.innerHTML = `
      <div class="gk-map__bg">
        <img src="assets/scenes/campus-map.svg" alt="校园地图" class="gk-map__svg"/>
      </div>
      <div class="gk-map__header">
        <h2 class="gk-map__title">🗺 星辰高中 · 校园探索</h2>
        <div class="gk-map__subtitle">记忆碎片 ${done}/${total} · 点击地图上的标记前往，完成全部主线任务揭开真相</div>
      </div>
      <button class="gk-map__close">✕ 关闭</button>
      <button class="gk-map__status">📊 我的状态</button>
      <div class="gk-map__canvas">
        ${markers}
      </div>
      ${allDone
        ? '<div class="gk-map__submit" data-dest="WishRevealPre"><div class="gk-map__submit-icon">📜</div><div>提交志愿表 · 真相揭晓</div></div>'
        : '<div class="gk-map__locked">🔒 集齐 '+total+' 块主线碎片后解锁志愿提交（已完成 '+done+'/'+total+'）</div>'}
    `;
    document.body.appendChild(overlay);
    // 节点点击 → 跳转
    overlay.querySelectorAll('.gk-map__marker[data-dest], .gk-map__submit[data-dest]').forEach(node => {
    ...
    // 锁定节点点击 → 显示原因（已在 tip 里，但加个抖动反馈）
    overlay.querySelectorAll('.gk-map__marker--locked').forEach(node => {
      node.addEventListener('click', (e) => {
        e.stopPropagation();
        node.classList.add('gk-map__marker--shake');
        setTimeout(() => node.classList.remove('gk-map__marker--shake'), 500);
      });
    });
  ...
  return {
    ...
    rollNpcScores, showNpcInteract, showNpcPicker, showRealScoreForm,
    // 关系/属性/解谜/存档提醒
    addRelation, relation, relationTier, addAttr, attr, feedback, meetsAttr, meetsRelation,
    solvePuzzle, isSolved, saveWarn, showSaveWarn, showShardFlash, showPhotoMontage, showVisions, statusPanelHtml,
  };
})();
window.GK = GK;
