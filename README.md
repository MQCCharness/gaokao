...
**完整功能清单（v0.3 三次迭代后）**：
- 🎬 失忆穿越主线（开场闪回 → 校园地图 → 5任务 → 碎片集齐 → 三结局 → 重玩）
- 🗺 SLG 风格全屏校园地图（真实场景缩略图节点 + 依赖链门控）
- 🧩 5 关解谜（查分/MBTI/理想/兴趣/导师，贴合主题，正负反馈）
- 🎭 12+ 角色（4主角团 + 8导师 + 3同学 + 3家人 + 2老师，各有关系值）
- 📊 关系值 + 属性系统（耐心/洞察/勇气/务实，影响结局）
- 🏀 3 个放松场景（篮球/星空/河川敷，配环境音）
- 🎙 21 条 edge-tts 配音（每位角色独立音色）
- 📊 现实路线（输入真实分数 → AI 估算位次）
- 🎮 SLG 标准操作（Ctrl快进/滚轮翻对话/Space推进）
- 📱 移动端适配（手机横屏可玩）
